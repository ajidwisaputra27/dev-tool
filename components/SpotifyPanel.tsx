...
function parseSpotifyLink(raw: string) {
  ...
  let m = raw.match(/open\.spotify\.com\/(playlist|album|track|artist|episode|show)\/([a-zA-Z0-9]+)/);
  if (m) { type = m[1]; id = m[2]; }
  if (!type) {
    m = raw.match(/spotify:(playlist|album|track|artist|episode|show):([a-zA-Z0-9]+)/);
    if (m) { type = m[1]; id = m[2]; }
  }
...
export default function SpotifyPanel({
  ...
  // modal
  const [modal, setModal] = useState<{ mode: ModalMode; playlist?: SpotifyPlaylist } | null>(null);
  ...
  function loadEmbed(link: string, playlist?: SpotifyPlaylist) {
    ...
    if (!url) { setError('link tidak dikenali.'); return; }
    setError('');
    setEmbedUrl(url);
    if (playlist) {
  ...
  function handlePointerDown(playlist: SpotifyPlaylist) {
    holdTimer.current = setTimeout(async () => {
      ...
      if (!res.ok) { setError('gagal menghapus playlist.'); return; }
      setPlaylists((prev) => prev.filter((x) => x.id !== playlist.id));
      if (activeId === playlist.id) { setActiveId(null); setEmbedUrl(null); }
    }, 600);
  }
  ...
  async function handleModalSave() {
    ...
    if (!parseSpotifyLink(url)) { setModalErr('link spotify tidak valid.'); return; }
    ...
    if (modal?.mode === 'add') {
      ...
      if (!res.ok) { setModalErr('error' in data && data.error ? data.error : 'gagal simpan'); return; }
      ...
      const res = await fetch(`/api/playlists/${modal.playlist.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ link: url, name: modalName.trim() || modal.playlist.name }),
      });
      const data = (await res.json()) as SpotifyPlaylist | { error?: string };
      if (!res.ok) { setModalErr('error' in data && data.error ? data.error : 'gagal update'); return; }
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <button type="button" className="board-chip" onClick={() => setModal(null)}>
                batal
              </button>
              <button type="button" className="board-chip active" onClick={() => void handleModalSave()}>
                simpan
              </button>
            </div>
          </div>
